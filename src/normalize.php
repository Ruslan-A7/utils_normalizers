...
/**
 * Аналогічно до `pathNormalize()` - нормалізує шлях відповідно до сепаратора директорій для поточної операційної системи,
 * з доданою підтримкою альтернативного зворотного ходу по директоріям.
 *
...
/**
 * Аналогічно до `nestingToPath()` - перетворює шлях вкладеності на файловий шлях для поточної ОС,
 * з доданою підтримкою альтернативного зворотного ходу по директоріям.
 *
 * Повертає коректний шлях замінивши всі:
 * - роздільники вкладеності згідно $ns константи NS (може бути `>` або `.`) - на справжній сепаратор директорій для поточної операційної системи,
 * - роздільники зворотної вкладеності згідно константи BNS (може бути `<` або `^`) - на `../` або `..\` в залежності від DS.
 *
 * @param string $nesting шлях вкладеності для перетворення
 * @param string|null $ns роздільник вкладеності, на випадок, якщо потрібно щоб він відрізнявся від константи NS (може бути `>` або `.`)
 ...
 * (наприклад: `^path>to>file.php` -> `../path/to/file.php`)
 */
function nestingToPathPlus(string $nesting, ?string $ns = null): string {
    return str_replace(BNS, '..' . DS, nestingToPath($nesting, $ns));
}
